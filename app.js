...
function migrateData(){
  const raw = JSON.parse(localStorage.getItem("habitV251"));
  if(!raw) return;
  Object.keys(raw).forEach(month=>{
    const m = raw[month];
    if(!m.habits) return;
    m.habits.forEach((h,i)=>{
      if(!h.id){
        h.id = "h"+(i+1);
      }
    });
    if(m.days){
      Object.keys(m.days).forEach(d=>{
        m.days[d] = m.days[d].map(name=>{
          const habit = m.habits.find(h=>h.name===name);
          return habit ? habit.id : name;
        });
      });
    }
  });
  localStorage.setItem("habitV251",JSON.stringify(raw));
}
migrateData();
...
{id:"h1",name:"Exercise",archived:false,type:"personal"},
{id:"h2",name:"Walk",archived:false,type:"personal"},
{id:"h3",name:"Read",archived:false,type:"personal"},
{id:"h4",name:"Podcast",archived:false,type:"personal"},
{id:"h5",name:"Sleep 7+",archived:false,type:"personal"},
{id:"h6",name:"Cold Shower",archived:false,type:"personal"},
{id:"h7",name:"No Alcohol",archived:false,type:"personal"},
{id:"h8",name:"Plan Day",archived:false,type:"work"}
];
...
function monthSelector(){const keys=monthKeys();return `<div class="card month-picker"><label><strong>Month</strong></label><select onchange="changeMonth(this.value)">${keys.map(key=>`<option value="${key}" ${key===selectedMonthKey?'selected':''}>${formatMonth(key)}</option>`).join("")}</select></div>`}
